(function(){
        window.H5lock = function(obj){
            ...
            this.chooseType = Number(window.sessionStorage.getItem('chooseType')) || obj.chooseType;
        };
        ...
        H5lock.prototype.storePass = function(psw) {// touchend结束之后对密码和状态的处理
            ...
            if (this.pswObj.step == 1) {
                if (this.checkPass(this.pswObj.fpassword, psw)) {
                    ...
                    //window.sessionStorage.setItem('passwordxx', JSON.stringify(this.pswObj.spassword));
                    //window.sessionStorage.setItem('chooseType', this.chooseType);

                    //记录input的value值，以便后端存储
                    var p1 = '';    //密码
                    for (var i = 0 ; i < psw.length ; i++) {
                        p1 += psw[i].index + psw[i].index;
                    }
                    //console.log(p1);
                    $(".nine_password").val(p1);    //9宫格绘制的密码
                } else {
        ...
        H5lock.prototype.updatePassword = function(){
            ...
            window.sessionStorage.removeItem('chooseType');
            this.pswObj = {};
        ...
        H5lock.prototype.initDom = function(){
            ...
            var str = '<h4 id="title" class="title">绘制图案</h4>'+
                      '<a id="updatePassword" style="position: absolute;right: 5px;top: 5px;color:#fff;font-size: 10px;display:none;">重置密码</a>'+
                      '<canvas id="canvas" width="300" height="300" style="display: inline-block;margin-left:auto;margin-right:auto;"></canvas>';
            wrap.setAttribute('style','position: absolute;top:0;left:0;right:0;bottom:0; text-align:center;');
            wrap.innerHTML = str;
            document.getElementById('nine_box').appendChild(wrap);
        }
        H5lock.prototype.init = function() {
            this.initDom();
            // this.pswObj = window.sessionStorage.getItem('passwordxx') ? {
            //     step: 2,
            //     spassword: JSON.parse(window.sessionStorage.getItem('passwordxx'))
            // } : {};
            this.pswObj = {};
            //console.log(this.pswObj);
            this.lastPoint = [];
