...
class uploadfile extends BaseController
{
    ...
    public function index() {
        ...
        if($start_time){
            $map[]='add_time>='.$start_time_stamp;
        }
        if($end_time){
            $map[]='add_time<='.$end_time_stamp;
        }
        ...
        if(!empty($map)){   //假如是搜索的话
            $list_data=Db::name('uploadfile')->where($map_str)->order('id desc')->paginate(config('paginate.list_rows'),false,['query'=>input('get.')]);
        }else{  //默认不搜索
            $list_data=Db::name('uploadfile')->order('id desc')->paginate(config('paginate.list_rows'),false,['query'=>input('get.')]);
        }
        $list_data_content=$list_data->toArray();  //转成数组后的实际数据

        $title_data=Db::query("SELECT COLUMN_NAME,COLUMN_COMMENT,DATA_TYPE,COLUMN_DEFAULT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'ys_uploadfile' AND TABLE_SCHEMA = '".config('database.database')."'");     //查出字段信息
    ...
    public function data_add(){
        ...
        if (Request::instance()->isPost()){     // 是否为 POST 请求，用于添加处理数据
            ...
            $data=Db::query("SELECT COLUMN_NAME,COLUMN_COMMENT,DATA_TYPE,COLUMN_DEFAULT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'ys_uploadfile' AND TABLE_SCHEMA = '".config('database.database')."'");     //查出字段信息
    ...
    public function data_edit(){
        ...
        if (Request::instance()->isPost()){ //假如为post请求，就异步修改访问权限
            ...
            $id=input('get.id');
            $edit_data=Db::name('uploadfile')->where('id',$id)->find();    //查询出对应id访问权限数据
    ...
    public function data_del(){
        ...
        if(input('?post.id_arr')){   //批量删除
            $map['id']=array('in',input('post.id_arr/a'));
            $url_list_result=Db::name(input('post.pre_table'))->where($map)->select();
            foreach ($url_list_result as $key => $value) {
                $del_file_result=delDirAndFile(ROOT_PATH . 'public' . $value['url']);   //删除实际图片文件
            }
            ...
            $url_li_result=Db::name(input('post.pre_table'))->where('id',input('post.id'))->find();
            $del_file_result=delDirAndFile(ROOT_PATH . 'public' . $url_li_result['url']);   //删除实际图片文件
