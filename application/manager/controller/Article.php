...
class Article extends BaseController
{
    ...
    public function index() {
        ...
        if(!empty($map)){   //假如是搜索的话
            $list_data=Db::name('article')->where($map_str)->order('id desc')->paginate(config('paginate.list_rows'),false,['query'=>input('get.')]);
        }else{  //默认不搜索
            $list_data=Db::name('article')->order('id desc')->paginate(config('paginate.list_rows'),false,['query'=>input('get.')]);
        }
        $list_data_content=$list_data->toArray();  //转成数组后的实际数据

        $title_data=Db::query("SELECT COLUMN_NAME,COLUMN_COMMENT,DATA_TYPE,COLUMN_DEFAULT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'ys_article' AND TABLE_SCHEMA = '".config('database.database')."'");     //查出字段信息
    ...
    public function data_add(){
        ...
        if (Request::instance()->isPost()){     // 是否为 POST 请求，用于添加处理数据
            ...
            $data=Db::query("SELECT COLUMN_NAME,COLUMN_COMMENT,DATA_TYPE,COLUMN_DEFAULT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'ys_article' AND TABLE_SCHEMA = '".config('database.database')."'");     //查出字段信息
    ...
    public function data_edit(){
        ...
        if (Request::instance()->isPost()){ //假如为post请求，就异步修改访问权限
            ...
            $id=input('get.id');
            $edit_data=Db::name('article')->where('id',$id)->find();    //查询出对应id访问权限数据
